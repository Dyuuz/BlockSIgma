...
async def run_predictions_for_chunk():
    ...
    """
    try:
        log_memory("12hr Prediction Current memory usage at the start of data training")
        ...
        for job_id, (label, assets) in chunks.items():
            ...
            async with results_lock:
                ...
                if all(jid in all_results for jid in EXPECTED_JOB_IDS_12hr):
                    ...
                    if (se_12hr is not True) or (se_12hr_buy is not True):
        ...
        log_memory("12hr prediction memory usage at the end of data training")
...
async def save_exit_12hr_buy_summary(session: AsyncSession, update_label="Closure"):
    ...
    try:
        ...
        del twelve_hrs_buy_summary, current_summary, last_instance, latest_predictions
        # del details
...
async def get_current_predictions() -> None:
    ...
    try:
        ...
        t1 = time.monotonic()
        # print(f"[12hr TIMER] Fetch symbols: {t1 - t0:.2f}s")
        ...
        t2 = time.monotonic()
        # print(f"[12hr TIMER] get_filtered_assetchunk_status + delete_disabled: {t2 - t1:.2f}s")
        ...
        t3 = time.monotonic()
        # print(f"[12hr TIMER] Binance 24hr ticker fetch: {t3 - t2:.2f}s")
        ...
        t4 = time.monotonic()
        # print(f"[12hr TIMER] Row update loop ({updated_count} rows): {t4 - t3:.2f}s")
        ...
        t5 = time.monotonic()
        # print(f"[12hr TIMER] Summary save: {t5 - t4:.2f}s")

        del tickers, all_prices, all_assets
        gc.collect()
        log_memory("12hr Current memory usage after successive update.")
        
        print(f"[12hr Refresh TIMER] TOTAL: {t5 - t0:.2f}s")
...
async def fetch_12hrs_summary(db: AsyncSession):
    ...
    acc_stmt = select(func.count(Prediction.id)).where(Prediction.achievement == "Reached")
...
async def fetch_12hrs_buy_summary(db: AsyncSession):
    ...
    acc_stmt = select(func.count(Prediction.id)).where(and_(
