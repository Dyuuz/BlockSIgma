...
from app.models.models import Symbol, Prediction4hr, FourHoursSummary, FourHoursBuySummary
...
from app.views.predictions.Hybrid_Model_4hr import main_model
...
from app.schema.model_schema import PredictionBase_4hr
...
supabase_key = os.getenv("SUPABASE_KEY") if os_supabase_key else settings.SUPABASE_KEY
supabase_url = os.getenv("SUPABASE_URL") if os_supabase_url else settings.SUPABASE_URL
...
CONFLICT_KEY = "symbol"
EXPECTED_JOB_IDS_4hr = ["job_chunk1", "job_chunk2", "job_chunk3"]
...
def get_filtered_assetchunk_status(asset_list):
    ...
    active_spot_symbols = [symbol['symbol'] for symbol in exchange_info['symbols'] if symbol['status'] == 'TRADING']
    chunk = []
    chunk_disabled = []
...
async def get_asset_chunks(rows):
    ...
    for idx, array in enumerate(arrays):
        asset_chunks = {f"Asset chunk {idx+1}": array for idx, array in enumerate(arrays)}
    ...
    try:
        async with AsyncSessionLocal() as session:
            prediction_result = await session.execute(
                select(Prediction4hr).where(Prediction4hr.symbol.in_(symbols_to_delete))
            )
            ...
            if disabled_predictions is not None:
                await session.execute(
                    delete(Prediction4hr).where(Prediction4hr.symbol.in_(symbols_to_delete))
                )
                await session.commit()
                print(f"[✔] Deleted {len(disabled_predictions)} rows from 4hr Prediction table.")
...
async def run_predictions_for_chunk():
    ...
    - Finalize and update the most recent 4-hour(buy) prediction summary.
    - Create and store a new 4-hour(buy) prediction summary record.
    ...
    try:
        ...
        print("🚀 Starting chunk predictions for 4hr...")
        all_results = {}
        final_results = []
        ...
        async with AsyncSessionLocal() as session:
            result = await session.execute(select(Prediction4hr.symbol, Prediction4hr.asset_name))
            rows = result.all()
            symbol_to_name = {symbol: asset_name for symbol, asset_name in rows}
        ...
        if not symbol_to_name:
            print("Prediction4hr table is empty.")
        ...
        for job_id, (label, assets) in chunks.items():
            ...
            async with results_lock:
                ...
                if all(jid in all_results for jid in EXPECTED_JOB_IDS_4hr):
                    ...
                    try:
                        ...
                        update_cols = {
                            ...
                            for col in Prediction4hr.__table__.columns
                    ...
                    print("[✔] All chunks finished. Saved to 4hr Prediction DB...")
                    all_results.clear()

                else:
                    print(f"Skipping with result lock - waiting on other chunks. [{job_id}]")

        async for db in get_db():
            four_hrs_summary = await fetch_4hrs_summary(db)
            await create_4hr_summary(db, four_hrs_summary)

        print("[✔] Successfully created four hours summary table")

        async for db in get_db():
            four_hrs_buy_summary = await fetch_4hrs_buy_summary(db)
            await create_4hr_buy_summary(db, four_hrs_buy_summary)

        print("[✔] Successfully created four hours buy summary table")
...
async def save_exit_4hr_summary(session: AsyncSession, update_label="Closure"):
    """
    Update the most recent 4-hour summary with final prediction results.

    This function:
    - Fetches the most recent entry from the FourHoursSummary table
    ...
    try:
        ...
        if not current_summary:
            await create_4hr_summary(session, four_hrs_summary)
            print("[✔] Successfully created a real-time four hours summary table")
            return
...
async def save_exit_4hr_buy_summary(session: AsyncSession, update_label="Closure"):
    """
    Update the most recent 4-hour buy summary with final prediction results.

    This function:
    - Fetches the most recent entry from the FourHoursBuySummary table
    - Retrieves the latest buy prediction details
    - Updates the number of predictions and accuracy from the current summary
    - Optionally appends the instance details based on the `update_label`
    ...
    try:
        ...
        if not current_summary:
            ...
            print("[✔] Successfully created a real-time four hours buy summary table")
            return
...
async def create_4hr_summary(session: AsyncSession, four_hrs_summary):
    """
    Create and save a 4-hour summary to the database.

    This function accepts a dictionary containing summary statistics
    for 4-hour "Buy" predictions, and saves the data into the 
    FourHoursSummary table. It includes:
    ...
    try:
        ...
        print(f"[✖] Failed to create 4hr summary: {e}")


async def create_4hr_buy_summary(session: AsyncSession, four_hrs_buy_summary):
    """
    Create and save a 4-hour buy summary to the database.
        
    This function accepts a dictionary containing summary statistics
    for 4-hour "Buy" predictions, and saves the data into the 
    FourHoursBuySummary table. It includes:
    - from, to, number of predictions & accuracy
    ...
    try:
        ...
        print(f"[✖] Failed to create 4hr buy summary: {e}")
...
async def get_current_predictions() -> List[str]:
    ...
    - Successive Update of 4hr Summary(accuracy) tables

    Logs progress and errors to the console.
    """
    global four_hour_job_state  # Tell Python to use the global variable
    if four_hour_job_state:
        print("[✖]4hour prediction is currently running!")
        return
    
    print(f"Starting to update 4hr latest prices...")
    all_prices = {}
    ...
    try:
        ...
        stmt = insert(Prediction4hr).values(predictions)
        update_cols = {
            col.name: getattr(stmt.excluded, col.name)
            for col in Prediction4hr.__table__.columns
            if col.name not in ("id", "created_at", "symbol")
        ...
        print(f"[✔]Successfully Upserted {len(predictions)} rows on 4hrs Prediction Table.")
...
@router.get("/four-hour-prediction")
async def fetch_latest_prediction():
    ...
    """
    await get_asset_chunks([])
    async with AsyncSessionLocal() as session:
        result = await session.execute(select(Prediction4hr))
        all_rows = result.scalars().all()

    if not all_rows:
        raise HTTPException(status_code=404, detail="Prediction table is empty.")

    formatted = []
    for row in sorted(all_rows, key=lambda x: x.symbol):
        model = PredictionBase_4hr.model_validate(row)
...
async def fetch_4hrs_summary(db: AsyncSession):
    """
    Fetch 4-hour prediction summary.

    This function retrieves summary statistics for all "Buy" predictions 
    in the predictions_v3_4hr prediction table. It calculates and returns:
    ...
    count_stmt = select(func.count(Prediction4hr.id))
...
async def fetch_4hrs_buy_summary(db: AsyncSession):
    """
    Fetch 4-hour buy prediction summary.
    ...
    count_stmt = select(Prediction4hr).where(
        or_(
            Prediction4hr.prediction_status == "Buy",
...
@router.get("/four-hours-summary")
async def get_4hrs_summary(db: AsyncSession = Depends(get_db)):
    """
    Retrieve 4-hour buy summary data.

    This endpoint fetches summarized prediction data for every 4-hour interval.
...
@router.get("/four-hours-buy-summary")
async def get_4hrs_buy_summary(db: AsyncSession = Depends(get_db)):
    """
    Retrieve 4-hour-buy summary data.
